...
public class PenguinFlyingBehaviour : MonoBehaviour
{
    ...
    private void HandleSlideInput()
    {
        ...
        {
            if (state.isFlying) { EndSlide(); }
            else { StartSlide(); }
        }

        // Only auto-stop slide when on ground and no input
        if (state.isFlying && state.isGrounded && Input.GetAxis("Horizontal") == 0)
        {
            EndSlide();
        }
    }
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (state.isFlying && state.slideStartTime < 0)
        {
            state.slideStartTime = Time.time;
        }
